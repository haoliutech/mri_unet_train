...
def create_baseline_images(patent_ids, 
    ...
    for label_fname in both:
        ...
        if label_.is_file() and prediction_.is_file() and original_.is_file():
            ...
            metrics_ = plot_results_from_prediction(original_=original_, 
                                                       label_=label_, 
...
def create_reference_images(patent_ids, crop_shape=None):
    ...
    for label_fname in both:
        ...
        if label_.is_file() and prediction_.is_file() and original_.is_file() and original_T1_.is_file():
            ...
            overwrite_window = get_cropped_window_dims(np.rot90(crop_npy_to_shape(prediction_)+crop_npy_to_shape(label_)))

            f, ax = plt.subplots(1,1, figsize=(16,10))
            _ = plot_results_from_prediction(original_=original_, 
                                                label_=zeros_, 
                                                prediction_=zeros_,
                                                ax=ax,
                                                show_title=False,
            ...
            f, ax = plt.subplots(1,1, figsize=(16,10))
            _ = plot_results_from_prediction(original_=original_T1_, 
                                                label_=zeros_, 
                                                prediction_=zeros_,
                                                ax=ax,
                                                show_title=False,
    ...
    for idx in df.index:
        ...
        for _, data in enumerate(val_loader):
            ...
            with torch.set_grad_enabled(is_train):
                ...
                f, ax = plt.subplots(1,1, figsize=(16,10))
                try:
                    metrics_ = plot_results_from_prediction(original_=inputs, 
                                                                label_=labels, 
